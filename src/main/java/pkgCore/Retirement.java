...
public class Retirement {
	...
	public double MonthlySavings() {
		...
		double pmt = Math.round(Math.abs(Retirement.PMT(dAnnualReturnRetired/12, iYearsRetired * 12, this.TotalAmountToSave(),0,false)*100)/100); // <-- this should be fixed to calculate the real pmt
		return pmt;
	}
	...
	public double TotalAmountToSave() {
		...
		double pv = Math.round(Math.abs(Retirement.PV(dAnnualReturnRetired/12, iYearsRetired*12, dRequiredIncome-dMonthlySSI, 0, false)*100)/100);
